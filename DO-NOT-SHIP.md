...
Last verified against `questions.json` v39 (5,579 questions) on 2026-09-09.
...
| **NYC questions the kid actually sees** | **285** (v39) | **「280多道写纽约的题」** |
...
**2,799 questions, 52% of the bank.** Pick from it rather than hand-picking screenshots. This is a
...
different number — the frozen number is whatever §1 records for the current bank version (285 as of
v39), and consistency matters more than precision here. Re-run this script when the bank changes and
...
**v37 (2026-09-08).** The 50 new questions moved four of these numbers. NYC rose 281 → 282: exactly one
new item matches the term list, `g4_v37_019` (why New York City's harbor made it a major port), in
...
**v38 (2026-09-08).** A fix-only release (64 `type: reading` repairs, 15 distractor replacements). It
added no questions, so none of the §1 numbers moved; this file was not re-verified at the time, which
is why the header above jumped v37 → v39.

**v39 (2026-09-09).** The 50 new questions moved four of these numbers. NYC rose 282 → 285: three new
items match the term list, all G4 `social_studies` — `g4_v39_012` (what a star marks on a New York
State map), `g4_v39_014` (what the Ellis Island inspection checked), and `g4_v39_015` (why the mouth
of the Hudson River suited Dutch trade). `social_studies` is again the only slice of the subject mix
that moved (103 → 106). NYC-including-explain rose 290 → 293, the same three items. Standards-coded
rose 4,466 → 4,516 because all 50 new questions carry a code. The Pre-K–G3 share fell 81.0% → 80.7%:
20 of the 50 landed inside that range (G1) and 30 outside it (G4/G5).
